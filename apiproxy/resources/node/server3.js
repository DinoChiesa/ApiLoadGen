...
//    server = restify.createServer(),
    ...
    modelSourceUrlPrefix = '/mukundha/testdata',
...
function retrieveLoadProfileForJob(ctx) {
  return (function (context) {
    ...
    if ( ! job || !job.metadata.connections.uses)  {
      return q.resolve(context);
    }
...
function invokeOneRequest(context) {
  ...
  //console.log("Current client url = " + JSON.stringify(client.url));
  {
        var scheme = job.defaultProperties.scheme;
        var host = job.defaultProperties.host;
        var headers = job.defaultProperties.headers;
    
        if ( req.overrideProperties ){
          if(  req.overrideProperties.scheme){
            scheme = req.overrideProperties.scheme;
          }
          if ( req.overrideProperties.host ){
            host = req.overrideProperties.host ;
          }
          if ( req.overrideProperties.headers){
            headers = req.overrideProperties.headers;
          }
        }

        if ( client.url.host != host || client.url.scheme != scheme || client.url.headers != headers){
          console.log('overiding the url');
            state.restClient = restify.createJsonClient({
            url: scheme + '://' + host,
            headers: headers,
            signRequest : function(r) { return state.headerInjector(r);}
        ...
        respHandler = function(e, httpReq, httpResp, obj) {
          ...
          if (e) {
            ...
            for (i=0, L=req.extracts.length; i<L; i++) {
              ...
              log.write(ex.compiledFn);
              // actually invoke the compiled fn
              try {
                console.log(obj);
                console.log(ex.compiledFn(obj));
                console.log(ex.valueRef);
                console.log(ctx.state.extracts);
...
app.post('/jobs/:jobid', // RegExp here failed for me.
            function(req, res) {
              var jobid = req.params.jobid,
...
app.listen(8001, function() {
  log.write('=======================================================');
  log.write('loadgen server start, listening: ' + app);
});
